...
void CCU63_vInit(void) {
	...
	CCU63_T12PR = 0x070B;  // load CCU63 T12 period register
	...
	CCU63_CC60SR = 0x070C;	// Load CCU63 capture/compare shadow
	...
	CCU63_CC61SR = 0x070C;	// Load CCU63 capture/compare shadow
	...
	CCU63_CC62SR = 0x070C;	// Load CCU63 capture/compare shadow
	...
	CCU63_T12DTC = 0x0741;	// load CCU63 dead time control register for
							// timer T12
...
_interrupt(CCU63_NodeI0_INT) void CCU63_viNodeI0(void) {
	...
	if (CCU63_IS & 0x0040)	// if CCU63_IS_T12OM
	{
		...
		// laden der Schattenregister mit den nächsten Werten der Sinus Wertetabelle
		// U/f Steuerung mit Spannungsboost skaliert Amplitude
		// konstante Verschiebung des Gleichanteils um HALF_Periodvalue
		// float-Rechnung nicht ersetzt weil 1.keine Rechenzeitprobleme/ Zeit nicht woanders nötig
		// 2.bei Verwendung von long würden durch die Skalierung 2 long-Rechenoperationen und eine float-Rechenoperationen hinzu kommen
		CCU63_vLoadChannelShadowRegister(CCU63_CHANNEL_0, table[(index + OFFSET_L1) % ARRAY_SIZE] * (0.0145 * frequency + 0.2) + (HALF_PERIODVALUE));
